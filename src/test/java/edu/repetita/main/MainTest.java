...
public class MainTest {
    ...
    public void testMain_noErrorsNoVerboseOutpaths_runDefoWithFractionalTime () throws Exception {
        ...
        Pattern regex = Pattern.compile(".*Next hops priority 1 \\(explicit paths\\)\\*\\*\\*.*\\*\\*\\*Next hops priority 2.*", Pattern.DOTALL);
        Matcher regexMatcher = regex.matcher(content);
        assert regexMatcher.find();
        Pattern regex1 = Pattern.compile(".*Next hops priority 2 \\(sr paths\\)\\*\\*\\*.*Destination.*", Pattern.DOTALL);
    ...
    public void testMain_noErrorsOutpaths_runAnotherExternalSolver () throws Exception {
        ...
        String content = new String(Files.readAllBytes(outfile));
        Files.delete(outfile);
        Pattern regex = Pattern.compile(".*Next hops priority 1 \\(explicit paths\\)\\*\\*\\*.*Destination.*", Pattern.DOTALL);
        Matcher regexMatcher = regex.matcher(content);
        assert regexMatcher.find();
    }
