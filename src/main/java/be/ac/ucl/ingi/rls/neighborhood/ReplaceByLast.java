...
public class ReplaceByLast extends Neighborhood<Integer> implements Trial {
    public final String name = "ReplaceByLast";
    ...
    private int storedDetour = lastDetour;

    public ReplaceByLast(PathState pathState, boolean debug) {
        this.pathState = pathState;
        this.debug = debug;
        this.maxDetourSize = pathState.maxDetourSize;
    }

    public ReplaceByLast(PathState pathState) {
    ...
    public void applyBest() {
        ...
        if (debug) {
            System.out.println("Replacing by " + lastDetour + " at position " + position + " for demand " + demand);
        }
        apply();
    }
}
