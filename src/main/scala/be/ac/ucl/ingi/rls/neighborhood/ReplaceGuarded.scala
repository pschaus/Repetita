...
class ReplaceGuarded(nNodes: Int, nEdges: Int, ecmp: ECMP, pathState: PathState, maxLoad: MaxLoad)(implicit debug: Boolean) 
extends Neighborhood[Demand] 
{
  val name = "ReplaceGuarded"
  ...
  override def apply() = {
    ...
    val middle = pathState.path(demand)(position)
    val dest   = pathState.path(demand)(position + 1)
    
    val loadSub = fractions(src)(middle)(maxEdge) + fractions(middle)(dest)(maxEdge)
    val loadAdd = fractions(src)(node)(maxEdge) + fractions(node)(dest)(maxEdge)

    nAttempts += 1
    if (loadSub > loadAdd) {
      nMade += 1
      // if ((nMade & 0xFFFF) == 0) println(s"$nAttempts replace attempts, $nMade really made")
